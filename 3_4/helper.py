...
def plot_voronoi(name,model,x,y,alpha=0.25):    
    ...
    plt.ylim(min(vor.min_bound) - 2, max(vor.max_bound) + 2)
    x_label,y_label = iris.feature_names[x],iris.feature_names[y]
    plt.xlabel(x_label)
    plt.ylabel(y_label)
    plt.legend()
    plt.title("Figura: regiones del modelo entrenado con %s para '%s' vs. '%s'" % \
               (name, x_label[:-4], y_label[:-4]),
